...
# Import your custom server-side session interface from supabase_session.py
# (Make sure you have supabase_session.py, and a 'flask_sessions' table in Supabase.)
...
# Use the custom session interface so large session data is stored in Supabase
...
def prepare_history():
    ...
    store the summary in session["log_summary"],
    keep only the last 5 lines as full detail.
    """
    log_list = session.get("interaction_log", [])
    if "log_summary" not in session:
        session["log_summary"] = ""  # aggregated memory

    # If we have more than 10 lines, let's summarize everything except the last 5
    if len(log_list) > 10:
        old_chunk = log_list[:-5]  # lines to summarize
        new_chunk = log_list[-5:]  # keep last 5 lines

        summary_text = summarize_lines(old_chunk)
        # append to the existing memory
        session["log_summary"] += "\n" + summary_text

        # now keep only 5 lines
...
def interpret_npc_state(affection, trust, npc_mood, current_stage, last_user_action, full_history=""):
    """
    This function is called after a user action.
    We will:
      1) Summarize older lines with prepare_history()
      2) Combine session["log_summary"] + last lines
      3) If user action is empty => "OOC: Continue the scene"
      4) Generate a response => includes AFFECT_CHANGE_FINAL, NARRATION, IMAGE_PROMPT
    """
    # 1) Summarize if needed
    prepare_history()

    # Merge memory summary + last lines
    memory_summary = session.get("log_summary","")
    # The "full_history" param is typically the entire session log joined
    # but we only need the last lines from session["interaction_log"] if you want
    # We'll just ignore "full_history" and do our own:
    recent_lines = session.get("interaction_log", [])
    combined_history = memory_summary + "\n" + "\n".join(recent_lines)

    # If user action is empty => treat as OOC: Continue scene
    if not last_user_action.strip():
        last_user_action = "OOC: Continue the scene"

    stage_label = STAGE_INFO.get(current_stage,{}).get("label","Unknown Stage")
    stage_desc = STAGE_INFO.get(current_stage,{}).get("desc","No description")
...
AFFECT_CHANGE_FINAL: ...
NARRATION: ...
IMAGE_PROMPT: single-sentence referencing NPC's age, ethnicity, body type, hair color/style, outfit, environment, etc.
...
NARRATION: [System: no valid response, please try again]
IMAGE_PROMPT: (error)
"""
...
def continue_session():
    # If you want to force reload from DB or something, you can do that here
...
def logout_route():
    ...
    auth_keys = ['logged_in', 'user_id', 'user_email', 'access_token']
...
@login_required
def save_game():
    flash("Example route - store a permanent game state if desired.","info")
    return redirect(url_for("interaction"))
...
def personalize():
    if request.method=="POST" and "save_personalization" in request.form:
        ...
        # also reset memory summary
        session["log_summary"] = ""
        flash("Personalization saved. Start your scene!", "success")
        return redirect(url_for("interaction"))
    else:
...
def interaction():
    if request.method=="GET":
        ...
        st_label = STAGE_INFO.get(cstage,{}).get("label","Unknown Stage")
        st_desc = STAGE_INFO.get(cstage,{}).get("desc","No description")
        ...
        if "submit_action" in request.form:
            user_action = request.form.get("user_action","")  # might be empty
            # interpret empty as "OOC: Continue the scene" in interpret_npc_state
            affection = session.get("affectionScore",0.0)
            trust = session.get("trustScore",5.0)
            mood = session.get("npcMood","Neutral")
            cstage = session.get("currentStage",1)
            ...
            # we used to do full_history=..., but we let interpret_npc_state handle summarizing
            full_history = "not used here"
            ...
            # We removed the old generate_scene_prompt route
...
STORY LOG:
{full_narration}
Now produce 600-900 words from the user's POV.
"""
    chat = model.start_chat()
